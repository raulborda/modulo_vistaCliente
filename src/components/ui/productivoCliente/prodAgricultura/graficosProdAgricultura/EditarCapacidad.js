...
export const EditarCapacidad = ({ cosechaActiva }) => {
  ...
  useEffect(() => {
    setDataContext({
      //*PROPIAS
      agricultura: Math.trunc(
        infoEdit.find(
          (info) => info.arubro_desc === "AGRICULTURA" && info.condicion === "P"
        ).has
      ),
      ganaderia: Math.trunc(
        infoEdit.find(
          (info) => info.arubro_desc === "GANADERIA" && info.condicion === "P"
        ).has
      ),
      tambo: Math.trunc(
        infoEdit.find(
          (info) => info.arubro_desc === "TAMBO" && info.condicion === "P"
        ).has
      ),
      mixto: Math.trunc(
        infoEdit.find(
          (info) => info.arubro_desc === "MIXTO" && info.condicion === "P"
        ).has
      ),

      //* ALQUILADAS
      agriculturaA: Math.trunc(
        infoEdit.find(
          (info) => info.arubro_desc === "AGRICULTURA" && info.condicion === "A"
        ).has
      ),
      ganaderiaA: Math.trunc(
        infoEdit.find(
          (info) => info.arubro_desc === "GANADERIA" && info.condicion === "A"
        ).has
      ),
      tamboA: Math.trunc(
        infoEdit.find(
          (info) => info.arubro_desc === "TAMBO" && info.condicion === "A"
        ).has
      ),
      mixtoA: Math.trunc(
        infoEdit.find(
          (info) => info.arubro_desc === "MIXTO" && info.condicion === "A"
        ).has
      ),
  ...
  return (
    <>
      <div className="cont">
        <div>
          <h1 className="titulos">CAPACIDAD PRODUCTIVA - EDITAR</h1>
        </div>
        <div>
          <table>
            <thead>
