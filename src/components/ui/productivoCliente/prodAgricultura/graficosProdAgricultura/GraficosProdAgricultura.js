...
const GraficosProdAgricultura = ({ cosechaActiva }) => {
  ...
  const [selectedValue, setSelectedValue] = useState(
    localStorage.getItem("cosechaActiva")
  );
    ...
    capacidad = [
      {
        ...
        alquiler: result.AGRICULTURA
          ? Math.trunc(result.AGRICULTURA.alquilado)
          : 0,
        total: result.AGRICULTURA
          ? parseInt(result.AGRICULTURA.propio) +
            parseInt(result.AGRICULTURA.alquilado)
          : 0,
        porcentaje: result.AGRICULTURA
          ? parseInt(infoCap[0]?.ahxs_propias || 0) +
              parseInt(infoCap[0]?.ahxs_alquiladas || 0) !==
            0
            ? (
                ((parseInt(result.AGRICULTURA.propio) +
                  parseInt(result.AGRICULTURA.alquilado)) /
      ...
      },
      {
        ...
        total: result.GANADERIA
          ? parseInt(result.GANADERIA.propio) +
            parseInt(result.GANADERIA.alquilado)
          : 0,
      ...
      },
      {
        ...
        porcentaje: result.TAMBO
          ? parseInt(infoCap[0]?.ahxs_propias || 0) +
            ...
            0
            ? (
                ((parseInt(result.TAMBO.propio) +
                  parseInt(result.TAMBO.alquilado)) /
                  (parseInt(infoCap[0]?.ahxs_propias || 0) +
                    parseInt(infoCap[0]?.ahxs_alquiladas || 0))) *
                100
              ).toFixed(0) + "%"
      ...
      },
      {
        ...
        porcentaje: result.MIXTO
          ? parseInt(infoCap[0]?.ahxs_propias || 0) +
            ...
            0
            ? (
                ((parseInt(result.MIXTO.propio) +
                  parseInt(result.MIXTO.alquilado)) /
                  (parseInt(infoCap[0]?.ahxs_propias || 0) +
                    parseInt(infoCap[0]?.ahxs_alquiladas || 0))) *
                100
              ).toFixed(0) + "%"
      ...
      },
      {
        ...
        propias: (
          <strong>
            {Math.trunc(
              infoCap && infoCap.length > 0 ? infoCap[0].ahxs_propias : 0
            )}
          </strong>
        ),
        alquiler: (
          <strong>
            {Math.trunc(
              infoCap && infoCap.length > 0 ? infoCap[0].ahxs_alquiladas : 0
            )}
          </strong>
        ),
        total: (
          <strong>
            {(infoCap && infoCap.length > 0
              ? parseInt(infoCap[0].ahxs_propias)
              : 0) +
              (infoCap && infoCap.length > 0
                ? parseInt(infoCap[0].ahxs_alquiladas)
                : 0)}
          </strong>
        ),
        porcentaje: (
        ...
        if (!capacidad || capacidad.length === 0) {
          return (
            <Empty
              ...
              description="No hay datos. Se puede cargar la cosecha activa"
            />
          );
        }
        return (
          <Table columns={columns} dataSource={capacidad} pagination={false} />
        );
      case 2:
  ...
  if (infoCap.length > 0 || selectedAcosDesc !== cosechaActiva) {
    //!Esto es para que solamente se pueda agregar cosecha si es la cosecha activa.
    setIsButtonDisabled(true);
  } else {
    setIsButtonDisabled(false);
  }
    ...
    <>
      ...
      >
        {cardSelected === 0 && (
          ...
          >
            <h1 className="titulos" style={{ margin: 0 }}>
